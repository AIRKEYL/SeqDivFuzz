...
def test(arglist):
    sia_model = predict_siamese.load_tapnet_mode()
    sia_model.cuda()
    sia_model.load_state_dict(torch.load(r'./tapnet/data/weights/tapnet.pkl'))
    sia_model.eval()
    ...
    fuzzer, trainers = sample_seeds(500)
    with U.single_threaded_session():
        ...
        noCrashF = open('./results/noCrashStateSeqV2.txt', mode='a')
        crashF = open('./results/crashStateSeqV2.txt', mode='a')
        ...
        while current_fuzz_time - start_fuzz_time < 3600 * 4 and len(fuzzer.corpus) > 0:
            # get action
            action_n = [agent.action(obs) for agent, obs in zip(trainers,obs_n)]
            # environment step
            new_obs_n, rew_n, done_n, info_n = env.step(action_n)
            ...
            # print('len(state_seqs): ',len(state_seqs))

            if (len(state_seqs) == Hyperparameter.Step):
                siameseTime1 = time.time()
                ret = predict_siamese.predict_once(sia_model, bench_noCrash, state_seqs)
            ...
            for i, rew in enumerate(rew_n):
                episode_rewards += rew
            if terminal and collisions > 5 and not done and len(state_seqs) >= Hyperparameter.Step:
                print('Found: ', len(fuzzer.result))
                fuzzer.add_crash(copy.deepcopy(init_state))

                if (len(state_seqs) != Hyperparameter.Step):
                    for i in range(len(state_seqs)):
                        crashF.write(str(state_seqs[i]))
                        crashF.write('\n')
                    crashF.write("######")
                    crashF.write('\n')
                    ...
                    for i in range(len(state_seqs)):
                        crashF_40.write(str(state_seqs[i]))
                        crashF_40.write('\n')
                    crashF_40.write("######")
                    crashF_40.write('\n')
            ...
            elif len(state_seqs) >= Hyperparameter.Step and (done or terminal):

                if (len(state_seqs) != Hyperparameter.Step):
                    for i in range(len(state_seqs)):
                        noCrashF.write(str(state_seqs[i]))
                        noCrashF.write('\n')
                    noCrashF.write("######")
                    noCrashF.write('\n')
                    ...
                    for i in range(len(state_seqs)):
                        noCrashF_40.write(str(state_seqs[i]))
                        noCrashF_40.write('\n')
                    noCrashF_40.write("######")
                    noCrashF_40.write('\n')
