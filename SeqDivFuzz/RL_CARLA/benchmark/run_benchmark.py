...
def run_single(env, weather, start, target, agent_maker, seed, replay=False, em_guide=True):
    ...
    if replay == True:
        ...
        print('fuzzing start!')
        crashF = open('/home/dell/wangkr/Siamese/MDPFuzz_Siamese/RL_CARLA/results/crashStateSeqV2.txt', mode='a')
        noCrashF = open('/home/dell/wangkr/Siamese/MDPFuzz_Siamese/RL_CARLA/results/noCrashStateSeqV2.txt', mode='a')

        crashF_40 = open('/home/dell/wangkr/Siamese/MDPFuzz_Siamese/RL_CARLA/results/crashStateSeqV2_40.txt', mode='a')
        noCrashF_40 = open('/home/dell/wangkr/Siamese/MDPFuzz_Siamese/RL_CARLA/results/noCrashStateSeqV2_40.txt', mode='a')
        timeStamp = open('/home/dell/wangkr/Siamese/MDPFuzz_Siamese/RL_CARLA/results/timeStamp.txt', mode='a')
        ...
        while len(env.fuzzer.corpus) > 0:
            ...
            while env.tick():
                ...
                if(len(sequence) == Hyperparameter.Step):

                    siameseTime1 = time.time()
                    ret = predict_siamese.predict_once(model, bench_noCrash, sequence)
                    siameseTime2 = time.time()
                    print('siameseTime: ', siameseTime2 - siameseTime1)
                    if ret == 1:
                        print('end')
                        break
                    else:
            ...
            if env.is_failure() or env.collided:
                ...
                if len(sequence) > Hyperparameter.Step+1:
                    for i in range(len(sequence)):
                        crashF.write(str(sequence[i]))
                        crashF.write('\n')
                    crashF.write("######")
                    crashF.write('\n')
                    ...
                    for i in range(len(sequence)):
                        crashF_40.write(str(sequence[i]))
                        crashF_40.write('\n')
                    crashF_40.write("######")
                    crashF_40.write('\n')
                ...
                if len(sequence) > Hyperparameter.Step+1:
                    ...
                    for i in range(len(sequence)):
                        noCrashF_40.write(str(sequence[i]))
                        noCrashF_40.write('\n')
                    noCrashF_40.write("######")
                    noCrashF_40.write('\n')
                    current_time = time.time()
            ...
            if end_fuzz_time - start_fuzz_time > 3600 * 4:
                break
